// Script: copia config de slots de uma página para outras páginas de uma máscara.
// Uso: node scripts/autocompletar_slots.mjs
// Requer: @supabase/supabase-js (já instalado no projeto)
...
async function main() {
    ...
    if (!prefRow) { console.error('❌ Nenhuma preferência salva para esta máscara. Configure a página de origem primeiro.'); process.exit(1); }

    const valor = prefRow.valor;
    console.log(`📋 ${Object.keys(valor).length} slots encontrados nas prefs.`);

    // 4. Filtrar slots da página de origem (ex: s2_*)
    ...
    // 5. Clonar para páginas destino (pula slots que já existem no banco)
    ...
    for (const pag of PAGINAS_DESTINO) {
        ...
        for (const [key, config] of slotsOrigem) {
            ...
            if (novoValor[novaChave] !== undefined) {
                pulados++;
        ...
        const msg = copiados > 0
            ? `📄 Página ${pag}: ${copiados} slot(s) copiados${pulados > 0 ? `, ${pulados} já existiam (ignorados)` : ''}.`
            : `📄 Página ${pag}: todos os ${pulados} slot(s) já estavam preenchidos — nada alterado.`;
        console.log(msg);
    }
