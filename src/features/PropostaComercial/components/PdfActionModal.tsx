...
export default function PdfActionModal({ blob, fileName, onClose, showView = true }: Props) {
    ...
    async function handleShare() {
        ...
        try {
            ...
            // AbortError = usuário cancelou; qualquer outro erro = tipo não suportado → baixa direto
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden">
                ...
                {/* Header */}
                <div className={`flex items-center gap-3 px-5 py-4 ${showView ? 'bg-orange-500' : 'bg-indigo-600'}`}>
                    <div className="w-10 h-10 bg-white/20 rounded-lg flex items-center justify-center shrink-0">
                        <span className="text-white text-lg">{showView ? '📄' : '📦'}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-white truncate">{fileName}</p>
                        <p className={`text-xs mt-0.5 ${showView ? 'text-orange-100' : 'text-indigo-200'}`}>
                            {showView ? 'PDF gerado com sucesso' : 'ZIP gerado com sucesso'}
                        </p>
                    </div>
                ...
                <div className="p-5 space-y-2.5">
                    ...
                    >
                        <span className="text-lg shrink-0">⬇</span>
                        <span>{showView ? 'Baixar PDF' : 'Baixar ZIP'}</span>
                    </button>
