...
const Layout: React.FC<LayoutProps> = ({ children, title, headerActions, mobileSidebarContent, onMobileBack, sidebarExtra }) => {
  ...
  const [sidebarExtraOpen, setSidebarExtraOpen] = React.useState(false);
        ...
        <div className="flex-1 overflow-y-auto">
          <nav className="px-4 space-y-0.5 md:space-y-1 mt-1 md:mt-4" onClick={() => setIsMenuOpen(false)}>
            ...
            {(user?.isAdmin || user?.isProjetista) ? (
              <>
                <button
                  onClick={(e) => { e.stopPropagation(); setSidebarExtraOpen(prev => !prev); }}
                  className={`flex items-center space-x-3 px-4 py-2 md:py-3 rounded-lg transition-all w-full text-left ${
                    sidebarExtraOpen
                      ? 'bg-blue-600 text-white shadow-md'
                      : 'text-slate-400 hover:bg-slate-100 hover:text-slate-700'
                  }`}
                >
                  ...
                  <span className="font-medium flex-1">Propostas</span>
                  <ChevronIcon className={`w-4 h-4 transition-transform duration-200 ${sidebarExtraOpen ? 'rotate-90' : ''}`} />
                </button>
                {sidebarExtraOpen && (
                  <div className="ml-4 mt-0.5 mb-1 space-y-0.5 border-l-2 border-blue-100 pl-3">
                    <button onClick={() => navigate('/propostas?modal=nova')} className="w-full text-left px-3 py-2 rounded-lg text-sm text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-colors">Nova Máscara</button>
                    <button onClick={() => navigate('/propostas?modal=editar')} className="w-full text-left px-3 py-2 rounded-lg text-sm text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-colors">Editar Máscara</button>
                    <button onClick={() => navigate('/propostas?modal=excluir')} className="w-full text-left px-3 py-2 rounded-lg text-sm text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-colors">Excluir Máscara</button>
                  </div>
                )}
              </>
            ) : (
...
const MascaraIcon = (props: any) => <svg {...props} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>;
const ChevronIcon = (props: any) => <svg {...props} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>;
